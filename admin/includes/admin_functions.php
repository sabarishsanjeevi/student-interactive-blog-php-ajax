...
function createAdmin($request_values){
	...
	if (count($errors) == 0) {
		...
		$query = "INSERT INTO users (username, name, email, phone, role, password, created_at, updated_at) 
				  VALUES('$username', '$name', '$email', '$phone', '$role', '$password', now(), now())";
...
{
	global $conn, $username, $role, $isEditingUser, $admin_id, $email ,$name , $phone;
...
function updateAdmin($request_values){
	...
	if (count($errors) == 0) {
		...
		$query = "UPDATE users SET username='$username',name='$name', email='$email', phone='$phone', role='$role', password='$password' WHERE id=$admin_id";
		mysqli_query($conn, $query);
